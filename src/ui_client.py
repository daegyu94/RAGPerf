...
with tab_main:
    ...
    with col1:
        config["bench"]["dataset"] = st.text_input(
            "Dataset Name", config["bench"]["dataset"]
        )
        data_types = ["text", "image", "audio"]
        config["bench"]["type"] = st.selectbox(
            "Data Type",
            data_types,
            index=data_types.index(config["bench"].get("type", "text")),
        )
        ...
        def on_slider_change():
            ...
            config["bench"]["preprocessing"]["dataset_ratio"] = st.session_state[
                key_slider
            ]
...
with tab_rag:
    ...
    with c2:
        ...
        actions["build_index"] = st.toggle(
            "4. Build Index", value=actions["build_index"]
        )
    with c3:
...
with tab_models:
    ...
    with col_a:
        ...
        ret["question_num"] = st.number_input(
            "Question Count", value=ret["question_num"]
        )
...
with tab_sys:
    ...
    with col_x:
        ...
        vdb["type"] = st.selectbox(
            ...
            index=(
                ["qdrant", "milvus", "lancedb", "chroma", "elasticsearch"].index(
                    vdb["type"]
                )
                if vdb["type"]
                in ["qdrant", "milvus", "lancedb", "chroma", "elasticsearch"]
                else 0
            ),
        )
        vdb["db_path"] = st.text_input("DB Path/URL", vdb["db_path"])
        vdb["collection_name"] = st.text_input(
            "Collection Name", vdb["collection_name"]
        )
