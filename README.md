...
The replay tool contains a Milvus client, not a Milvus server. Run `./scripts/milvus-standalone.sh start` before the first local test. The same endpoint can be used for source and target, with a different collection name for replay. Use separate deployments only when comparing environments.
