...
Use the [Milvus trace guide](milvus_trace/README.md) to record the requests generated by a complete RAG run and replay them against a separate Milvus endpoint. The guide covers installation choices, a local recorder smoke test, Text/Image/Audio examples, native replay, Docker replay, artifact validation, and troubleshooting.

The replay tool contains a Milvus client, not a Milvus server. Start the source and target Milvus deployments before following the guide.
