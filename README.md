...
Use the [Milvus trace guide](milvus_trace/README.md) to record the requests generated by a complete RAG run and replay them against a Milvus endpoint. The guide covers standalone server preparation, an optional artifact-format check, the actual reduced-scale record→replay smoke test, Text/Image/Audio examples, native replay, Docker replay, artifact validation, and troubleshooting.

The replay tool contains a Milvus client, not a Milvus server. Start one standalone Milvus server before the first test; the same endpoint can be used for source and target, with a different collection name for replay. Use separate deployments only when comparing environments.
